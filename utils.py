...
def load_datasets(data_path, split:str="train", streaming_name_whitelist=["translation", "mc4"]):
    ...
    datasets = []
    for data_path_name in data_path_names.split("+"):
        dataset_name, dataset_config = data_path_name.split("_", maxsplit=1)
        if "[" in dataset_config:
            dataset_config, dataset_range = dataset_config[:dataset_config.index("[")], dataset_config[dataset_config.index("["):]
        else:
            dataset_range = "[:]"
        if dataset_name in streaming_name_whitelist:
            dataset = load_dataset(os.path.join(data_path_base, dataset_name), config=dataset_config, split=split, streaming=True)
